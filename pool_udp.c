...
{
    pArg arg = (pArg)parg;
    sendto(arg->fd, arg->buf, arg->len, 0, (struct sockaddr *)arg->pclient_addr, sizeof(struct sockaddr));
	mem_free(arg->buf);
...
int main(int argc, char **argv)
{
    ...
    {
		char* buf= mem_malloc();
		int ret = recvfrom(listener, buf, MAXBUF, 0, (struct sockaddr *)&client_addr, &addr_size);
