...
#define TABLEINDEX(idx) (idx & ((1 << TABLEBIT) - 1))
...
void parseArg(int argc, char **argv) {
  if (argc < 5) {
    printf("Usage: %s {dstip} {dstport} {interval(microsecond)} "
           "{datalen(24-100)} {count} \n",
           argv[0]);
    exit(0);
  }
...
void Exit(int signal_id) { exit(0); }
...
void sendData(int signal_id) {
  ...
  srand(time(NULL));
  for (int len = sizeof(s_luSentCount) + sizeof(struct timeval);
       len < s_iDataLen; len++) {
    buf[len] = rand() % 57 + 65;
  }
  ...
  if (-1 == sendto(s_iSock, buf, s_iDataLen, 0, (struct sockaddr *)&s_servaddr,
                   sizeof(s_servaddr))) {
    ERR_EXIT("sendto");
  }
...
void recvData(void) {
  ...
  printf("cur=%ld.%ld, seq=%lu, time=%.2lf ms\n", tv.tv_sec, tv.tv_usec, seq,
         (tv.tv_sec - start.tv_sec) * 1000 +
             (double)(tv.tv_usec - start.tv_usec) / 1000);
}
